...
class ActionQuestion extends Action
{
    ...
    public function Init()
    {
        ...
        LS::Make(ModuleLang::class)->AddLangJs(
            [
                'topic_question_create_answers_error_max',
                'delete'
            ]
        );
    }
    ...
    protected function SubmitAdd()
    {
        ...
        foreach (getRequest('answer', []) as $sAnswer) {
    ...
    protected function checkTopicFields($oTopic)
    {
        ...
        if ($oTopic->getQuestionCountVote() == 0) {
            ...
            foreach ($aAnswers as $key => $sAnswer) {
                ...
                if (!func_check($sAnswer, 'text', 1, 100)) {
                    LS::Make(ModuleMessage::class)->AddError(
                        LS::Make(ModuleLang::class)->Get('topic_question_create_answers_error'),
                        LS::Make(ModuleLang::class)->Get('error')
                    );
                    $bOk = false;
                    break;
            ...
            if (count($aAnswers) < 2) {
                LS::Make(ModuleMessage::class)->AddError(
                    LS::Make(ModuleLang::class)->Get('topic_question_create_answers_error_min'),
                    LS::Make(ModuleLang::class)->Get('error')
                );
                $bOk = false;
            }
            if (count($aAnswers) > 20) {
                LS::Make(ModuleMessage::class)->AddError(
                    LS::Make(ModuleLang::class)->Get('topic_question_create_answers_error_max'),
                    LS::Make(ModuleLang::class)->Get('error')
                );
                $bOk = false;
            }
         ...
         * Выполнение хуков
         */
        LS::Make(ModuleHook::class)->Run('check_question_fields', ['bOk' => &$bOk]);
