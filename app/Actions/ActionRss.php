...
class ActionRss extends Action
{
    ...
    protected function RssComments()
    {
         ...
         */
        $aResult = LS::Make(ModuleComment::class)->GetCommentsAll(
            'topic',
            1,
            Config::Get('module.comment.per_page') * 2,
            [],
            $aCloseBlogs
        );
    ...
    protected function RssTopicComments()
    {
        ...
        if (!($oTopic = LS::Make(ModuleTopic::class)->GetTopicById($sTopicId)) or !$oTopic->getPublish()
            or $oTopic->getBlog()->getType() == 'close'
        ) {
         ...
         * Получаем комментарии
         */
        $aResult = LS::Make(ModuleComment::class)->GetCommentsByFilter(
            ['target_id' => $oTopic->getId(), 'target_type' => 'topic', 'delete' => 0],
            ['comment_id' => 'desc'],
            1,
            100
        );
    ...
    protected function RssTag()
    {
         ...
         */
        $aResult =
            LS::Make(ModuleTopic::class)->GetTopicsByTag($sTag, 1, Config::Get('module.topic.per_page') * 2, false);
        $aTopics = $aResult['collection'];
        /**
         * Формируем данные канала RSS
         */
    ...
    protected function RssColectiveBlog()
    {
         ...
         */
        if (!$sBlogUrl or !($oBlog = LS::Make(ModuleBlog::class)->GetBlogByUrl($sBlogUrl)) or $oBlog->getType()
            == "close"
        ) {
            parent::EventNotFound();
