...
class ActionSettings extends Action
{
    ...
    protected function EventUploadFoto()
    {
        ...
        if ($sFile = LS::Make(ModuleImage::class)->Resize(
            $sFileTmp,
            $sDir,
            'original',
            ...
            if ($sFilePreview = LS::Make(ModuleImage::class)->Resize(
                $sFileTmp,
                $sDir,
                'preview',
                Config::Get('view.img_max_width'),
                ...
                LS::Make(ModuleViewer::class)->AssignAjax(
                    'sTmpFile',
                    LS::Make(ModuleImage::class)->GetWebPath($sFilePreview)
                );
        ...
        LS::Make(ModuleMessage::class)->AddError(
            LS::Make(ModuleImage::class)->GetLastError(),
            LS::Make(ModuleLang::class)->Get('error')
        );
        unlink($sFileTmp);
    }
    ...
    protected function EventResizeFoto()
    {
        ...
        if ($aSizeFile = getimagesize($sFile) and isset($aSizeFile[0])) {
            $fRation =
                $aSizeFile[0] / 400; // 200 - размер превью по которой пользователь определяет область для ресайза
            if ($fRation < 1) {
                $fRation = 1;
            }
        }
        ...
        if (isset($aSizeTmp['x']) and is_numeric($aSizeTmp['x'])
            ...
            $aSize = [
                'x1' => round($fRation * $aSizeTmp['x']),
                'y1' => round($fRation * $aSizeTmp['y']),
                'x2' => round($fRation * $aSizeTmp['x2']),
                'y2' => round($fRation * $aSizeTmp['y2'])
            ];
        }
        /**
         * Вырезаем аватарку
        ...
        if ($sFileWeb = LS::Make(ModuleUser::class)->UploadFoto($sFile, $this->oUserCurrent, $aSize)) {
            ...
            LS::Make(ModuleViewer::class)->AssignAjax(
                'sTitleUpload',
                LS::Make(ModuleLang::class)->Get('settings_profile_photo_change')
            );
        } else {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('settings_profile_avatar_error'),
                LS::Make(ModuleLang::class)->Get('error')
            );
        }
    }
    ...
    protected function EventRemoveFoto()
    {
        ...
        LS::Make(ModuleViewer::class)->AssignAjax(
            'sTitleUpload',
            LS::Make(ModuleLang::class)->Get('settings_profile_photo_upload')
        );
    }
    ...
    protected function EventUploadAvatar()
    {
        ...
        if ($sFileAvatar = LS::Make(ModuleImage::class)->Resize(
            $sFileTmp,
            $sDir,
            'original',
            Config::Get('view.img_max_width'),
            ...
            LS::Make(ModuleViewer::class)->AssignAjax(
                'sTmpFile',
                LS::Make(ModuleImage::class)->GetWebPath($sFileAvatar)
            );
        } else {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleImage::class)->GetLastError(),
    ...
    protected function EventResizeAvatar()
    {
        ...
        if (isset($aSizeTmp['x']) and is_numeric($aSizeTmp['x'])
            ...
            $aSize = ['x1' => $aSizeTmp['x'], 'y1' => $aSizeTmp['y'], 'x2' => $aSizeTmp['x2'], 'y2' => $aSizeTmp['y2']];
        }
        /**
         * Вырезаем аватарку
         */
        if ($sFileWeb = LS::Make(ModuleUser::class)->UploadAvatar($sFileAvatar, $this->oUserCurrent, $aSize)) {
            ...
            LS::Make(ModuleViewer::class)->AssignAjax(
                'sTitleUpload',
                LS::Make(ModuleLang::class)->Get('settings_profile_avatar_change')
            );
        } else {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('settings_profile_avatar_error'),
    ...
    protected function EventRemoveAvatar()
    {
        ...
        LS::Make(ModuleViewer::class)->AssignAjax(
            'sTitleUpload',
            LS::Make(ModuleLang::class)->Get('settings_profile_avatar_upload')
        );
    }
    ...
    protected function EventTuning()
    {
        ...
        if (isPost('submit_settings_tuning')) {
             ...
             */
            LS::Make(ModuleHook::class)->Run('settings_tuning_save_before', ['oUser' => $this->oUserCurrent]);
            if (LS::Make(ModuleUser::class)->Update($this->oUserCurrent)) {
                LS::Make(ModuleMessage::class)->AddNoticeSingle(
                    LS::Make(ModuleLang::class)->Get('settings_tuning_submit_ok')
                );
                LS::Make(ModuleHook::class)->Run('settings_tuning_save_after', ['oUser' => $this->oUserCurrent]);
            } else {
                LS::Make(ModuleMessage::class)->AddErrorSingle(LS::Make(ModuleLang::class)->Get('system_error'));
            }
        } else {
    ...
    protected function EventInvite()
    {
        ...
        if (isPost('submit_invite')) {
             ...
             */
            if (!LS::Make(ModuleACL::class)->CanSendInvite($this->oUserCurrent)
                and !$this->oUserCurrent->isAdministrator()
            ) {
                LS::Make(ModuleMessage::class)->AddError(
                    LS::Make(ModuleLang::class)->Get('settings_invite_available_no'),
                    LS::Make(ModuleLang::class)->Get('error')
                );
                $bError = true;
            }
            ...
            if (!func_check(getRequestStr('invite_mail'), 'mail')) {
                LS::Make(ModuleMessage::class)->AddError(
                    LS::Make(ModuleLang::class)->Get('settings_invite_mail_error'),
                    LS::Make(ModuleLang::class)->Get('error')
                );
                $bError = true;
            }
             ...
             * Запускаем выполнение хуков
             */
            LS::Make(ModuleHook::class)->Run('settings_invate_send_before', ['oUser' => $this->oUserCurrent]);
            /**
            ...
            if (!$bError) {
                ...
                LS::Make(ModuleMessage::class)->AddNoticeSingle(
                    LS::Make(ModuleLang::class)->Get('settings_invite_submit_ok')
                );
                LS::Make(ModuleHook::class)->Run('settings_invate_send_after', ['oUser' => $this->oUserCurrent]);
            }
        }

        LS::Make(ModuleViewer::class)->Assign(
            'iCountInviteAvailable',
            LS::Make(ModuleUser::class)->GetCountInviteAvailable($this->oUserCurrent)
        );
        LS::Make(ModuleViewer::class)->Assign(
            'iCountInviteUsed',
            LS::Make(ModuleUser::class)->GetCountInviteUsed($this->oUserCurrent->getId())
        );
    }
    ...
    protected function EventAccount()
    {
        ...
        if (isPost('submit_account_edit')) {
            ...
            if (func_check(getRequestStr('mail'), 'mail')) {
                if ($oUserMail = LS::Make(ModuleUser::class)->GetUserByMail(getRequestStr('mail'))
                    and $oUserMail->getId() != $this->oUserCurrent->getId()
                ) {
                    LS::Make(ModuleMessage::class)->AddError(
                        LS::Make(ModuleLang::class)->Get('settings_profile_mail_error_used'),
                        LS::Make(ModuleLang::class)->Get('error')
                    );
                    $bError = true;
                }
                ...
                LS::Make(ModuleMessage::class)->AddError(
                    LS::Make(ModuleLang::class)->Get('settings_profile_mail_error'),
                    LS::Make(ModuleLang::class)->Get('error')
                );
                $bError = true;
            }
            ...
            if (getRequestStr('password', '') != '') {
                if (func_check(getRequestStr('password'), 'password', 5)) {
                    if (getRequestStr('password') == getRequestStr('password_confirm')) {
                        if (LS::Make(ModuleCrypto::class)->PasswordVerify(
                            getRequestStr('password_now'),
                            $this->oUserCurrent->getPassword()
                        )
                        ) {
                            ...
                            $bError = true;
                            LS::Make(ModuleMessage::class)->AddError(
                                LS::Make(ModuleLang::class)->Get('settings_profile_password_current_error'),
                                LS::Make(ModuleLang::class)->Get('error')
                            );
                        }
                    } else {
                        $bError = true;
                        LS::Make(ModuleMessage::class)->AddError(
                            LS::Make(ModuleLang::class)->Get('settings_profile_password_confirm_error'),
                            LS::Make(ModuleLang::class)->Get('error')
                        );
                    }
                } else {
                    $bError = true;
                    LS::Make(ModuleMessage::class)->AddError(
                        LS::Make(ModuleLang::class)->Get('settings_profile_password_new_error'),
                        LS::Make(ModuleLang::class)->Get('error')
                    );
                }
            }
            ...
            LS::Make(ModuleHook::class)->Run(
                'settings_account_save_before',
                ['oUser' => $this->oUserCurrent, 'bError' => &$bError]
            );
            /**
             * Сохраняем изменения
             */
            if (!$bError) {
                if (LS::Make(ModuleUser::class)->Update($this->oUserCurrent)) {
                    LS::Make(ModuleMessage::class)->AddNoticeSingle(
                        LS::Make(ModuleLang::class)->Get('settings_account_submit_ok')
                    );
                    /**
                    ...
                    if (getRequestStr('mail') and getRequestStr('mail') != $this->oUserCurrent->getMail()) {
                        if ($oChangemail =
                            LS::Make(ModuleUser::class)->MakeUserChangemail($this->oUserCurrent, getRequestStr('mail'))
                        ) {
                            if ($oChangemail->getMailFrom()) {
                                LS::Make(ModuleMessage::class)->AddNotice(
                                    LS::Make(ModuleLang::class)->Get('settings_profile_mail_change_from_notice')
                                );
                            } else {
                                LS::Make(ModuleMessage::class)->AddNotice(
                    ...
                    LS::Make(ModuleHook::class)->Run('settings_account_save_after', ['oUser' => $this->oUserCurrent]);
                } else {
                    LS::Make(ModuleMessage::class)->AddErrorSingle(LS::Make(ModuleLang::class)->Get('system_error'));
                }
            }
    ...
    protected function EventProfile()
    {
        ...
        LS::Make(ModuleViewer::class)->Assign(
            'aUserFieldsContact',
            LS::Make(ModuleUser::class)->getUserFields(['contact', 'social'])
        );
        /**
        ...
        if (isPost('submit_profile_edit')) {
             ...
             * Проверяем пол
             */
            if (in_array(getRequestStr('profile_sex'), ['man', 'woman', 'other'])) {
             ...
             */
            if (func_check(getRequestStr('profile_birthday_day'), 'id', 1, 2) and func_check(
                    getRequestStr('profile_birthday_month'),
                    'id',
                    1,
                    2
                ) and func_check(getRequestStr('profile_birthday_year'), 'id', 4, 4)
            ) {
            ...
            if (func_check(getRequestStr('profile_about'), 'text', 1, 3000)) {
                $this->oUserCurrent->setProfileAbout(
                    LS::Make(ModuleText::class)->Parser(getRequestStr('profile_about'))
                );
            } else {
                ...
                if (LS::Make(ModuleUser::class)->Update($this->oUserCurrent)) {
                    ...
                    if (is_array($aFieldsContactType)) {
                        foreach ($aFieldsContactType as $k => $v) {
                            $v = (string)$v;
                            if (isset($aFields[$v]) and isset($aFieldsContactValue[$k]) and is_string(
                                    $aFieldsContactValue[$k]
                                )
                            ) {
                                LS::Make(ModuleUser::class)->setUserFieldsValues(
                                    $this->oUserCurrent->getId(),
                                    [$v => $aFieldsContactValue[$k]],
                    ...
                    LS::Make(ModuleMessage::class)->AddNoticeSingle(
                        LS::Make(ModuleLang::class)->Get('settings_profile_submit_ok')
                    );
                    LS::Make(ModuleHook::class)->Run('settings_profile_save_after', ['oUser' => $this->oUserCurrent]);
                } else {
                    LS::Make(ModuleMessage::class)->AddErrorSingle(LS::Make(ModuleLang::class)->Get('system_error'));
                }
            }
         ...
         */
        $aCountries = LS::Make(ModuleGeo::class)->GetCountries([], ['sort' => 'asc'], 1, 300);
        LS::Make(ModuleViewer::class)->Assign('aGeoCountries', $aCountries['collection']);
        if ($oGeoTarget) {
            if ($oGeoTarget->getCountryId()) {
                $aRegions = LS::Make(ModuleGeo::class)->GetRegions(
                    ['country_id' => $oGeoTarget->getCountryId()],
                    ['sort' => 'asc'],
                    1,
                    500
                );
            ...
            if ($oGeoTarget->getRegionId()) {
                $aCities = LS::Make(ModuleGeo::class)->GetCities(
                    ['region_id' => $oGeoTarget->getRegionId()],
                    ['sort' => 'asc'],
                    1,
                    500
                );
    ...
    public function EventShutdown()
    {
        $iCountTopicFavourite =
            LS::Make(ModuleTopic::class)->GetCountTopicsFavouriteByUserId($this->oUserCurrent->getId());
        $iCountTopicUser = LS::Make(ModuleTopic::class)->GetCountTopicsPersonalByUser($this->oUserCurrent->getId(), 1);
        $iCountCommentUser =
            LS::Make(ModuleComment::class)->GetCountCommentsByUserId($this->oUserCurrent->getId(), 'topic');
        $iCountCommentFavourite =
            LS::Make(ModuleComment::class)->GetCountCommentsFavouriteByUserId($this->oUserCurrent->getId());
        ...
        LS::Make(ModuleViewer::class)->Assign(
            'iCountWallUser',
            LS::Make(ModuleWall::class)->GetCountWall(['wall_user_id' => $this->oUserCurrent->getId(), 'pid' => null])
        );
        /**
        ...
        LS::Make(ModuleViewer::class)->Assign(
            'iCountFriendsUser',
            LS::Make(ModuleUser::class)->GetCountUsersFriend($this->oUserCurrent->getId())
        );
