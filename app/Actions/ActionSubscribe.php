...
class ActionSubscribe extends Action
{
    ...
    protected function EventUnsubscribe()
    {
         ...
         */
        if ($oSubscribe = LS::Make(ModuleSubscribe::class)->GetSubscribeByKey($this->getParam(0))
            and $oSubscribe->getStatus() == 1
        ) {
            /**
            ...
            LS::Make(ModuleMessage::class)->AddNotice(
                LS::Make(ModuleLang::class)->Get('subscribe_change_ok'),
                null,
                true
            );
        }
         ...
         */
        if ((!$sUrl =
            LS::Make(ModuleSubscribe::class)->GetUrlTarget($oSubscribe->getTargetType(), $oSubscribe->getTargetId()))
        ) {
    ...
    protected function EventAjaxSubscribeToggle()
    {
        ...
        if (!func_check($sMail, 'mail')) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('registration_mail_error'),
