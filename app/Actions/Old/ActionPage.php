...
class ActionPage extends Action
{
    ...
    protected function EventAdmin()
    {
        ...
        if ($this->GetParam(0) == 'edit') {
            if ($oPageEdit = LS::Make(ModuleStaticPage::class)->GetPageById($this->GetParam(1))) {
                ...
                LS::Make(ModuleMessage::class)->AddError(
                    LS::Make(ModuleLang::class)->Get('page.edit_notfound'),
        ...
        if ($this->GetParam(0) == 'delete') {
            ...
            if (LS::Make(ModuleStaticPage::class)->deletePageById($this->GetParam(1))) {
                LS::Make(ModuleMessage::class)->AddNotice(
                    LS::Make(ModuleLang::class)->Get('page.admin_action_delete_ok')
                );
            } else {
                LS::Make(ModuleMessage::class)->AddError(
                    LS::Make(ModuleLang::class)->Get('page.admin_action_delete_error'),
                    LS::Make(ModuleLang::class)->Get('error')
                );
            }
        }
         ...
         */
        if ($this->GetParam(0) == 'sort' and
            $oPage = LS::Make(ModuleStaticPage::class)->GetPageById($this->GetParam(1))
        ) {
            LS::Make(ModuleSecurity::class)->ValidateSendForm();
    ...
    protected function CheckPageFields()
    {
        ...
        if (!func_check(getRequest('page_title', null, 'post'), 'text', 2, 200)) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('page.create_title_error'),
                LS::Make(ModuleLang::class)->Get('error')
            );
            $bOk = false;
        }
        ...
        if (!func_check(getRequest('page_url', null, 'post'), 'login', 1, 50)) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('page.create_url_error'),
                LS::Make(ModuleLang::class)->Get('error')
            );
            $bOk = false;
        }
        ...
        if (in_array(getRequest('page_url', null, 'post'), $this->aBadPageUrl)) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('page.create_url_error_bad').' '.join(',', $this->aBadPageUrl),
                LS::Make(ModuleLang::class)->Get('error')
            );
            $bOk = false;
        }
        ...
        if (!func_check(getRequest('page_text', null, 'post'), 'text', 1, 50000)) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('page.create_text_error'),
                LS::Make(ModuleLang::class)->Get('error')
            );
            $bOk = false;
        }
         ...
         */
        if (getRequest('page_pid') != 0 and !($oPageParent =
                LS::Make(ModuleStaticPage::class)->GetPageById(getRequest('page_pid')))
        ) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('page.create_parent_page_error'),
                LS::Make(ModuleLang::class)->Get('error')
            );
            $bOk = false;
        }
        ...
        if (getRequest('page_sort') and !is_numeric(getRequest('page_sort'))) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('page.create_sort_error'),
                LS::Make(ModuleLang::class)->Get('error')
            );
            $bOk = false;
        }
         ...
         * Выполнение хуков
         */
        LS::Make(ModuleHook::class)->Run('check_page_fields', ['bOk' => &$bOk]);
