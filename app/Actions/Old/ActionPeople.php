...
class ActionPeople extends Action
{
    ...
    protected function EventAjaxSearch()
    {
         ...
         * Ищем пользователей
         */
        $aResult = LS::Make(ModuleUser::class)->GetUsersByFilter(
            ['activate' => 1, 'login' => $sTitle],
            ['user_rating' => 'desc'],
            1,
            50
        );
    ...
    protected function EventCountry()
    {
         ...
         */
        $aResult = LS::Make(ModuleGeo::class)->GetTargets(
            ['country_id' => $oCountry->getId(), 'target_type' => 'user'],
            $iPage,
    ...
    protected function EventCity()
    {
         ...
         */
        $aResult = LS::Make(ModuleGeo::class)->GetTargets(
            ['city_id' => $oCity->getId(), 'target_type' => 'user'],
            $iPage,
            Config::Get('module.user.per_page')
        );
        $aUsersId = [];
