...
class ActionStream extends Action
{
    ...
    public function Init()
    {
        ...
        LS::Make(ModuleLang::class)->AddLangJs(
            [
                'stream_subscribes_already_subscribed',
                'error'
            ]
        );
    }
    ...
    protected function EventUser()
    {
        ...
        LS::Make(ModuleViewer::class)->Assign(
            ...
            LS::Make(ModuleStream::class)->GetCountByReaderId($this->oUserCurrent->getId()) < Config::Get(
    ...
    protected function EventAll()
    {
        ...
        LS::Make(ModuleViewer::class)->Assign(
            'bDisableGetMoreButton',
            LS::Make(ModuleStream::class)->GetCountAll() < Config::Get('module.stream.count_default')
        );
        LS::Make(ModuleViewer::class)->Assign('aStreamEvents', $aEvents);
        if (count($aEvents)) {
            $oEvenLast = end($aEvents);
    ...
    protected function EventSwitchEventType()
    {
        ...
        LS::Make(ModuleMessage::class)->AddNotice(
            LS::Make(ModuleLang::class)->Get('stream_subscribes_updated'),
    ...
    protected function EventSubscribe()
    {
        ...
        if ($this->oUserCurrent->getId() == getRequestStr('id')) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get('stream_error_subscribe_to_yourself'),
                LS::Make(ModuleLang::class)->Get('error')
            );
    ...
    protected function EventSubscribeByLogin()
    {
        ...
        if (!$oUser) {
            LS::Make(ModuleMessage::class)->AddError(
                LS::Make(ModuleLang::class)->Get(
                    'user_not_found',
                    ['login' => htmlspecialchars(getRequestStr('login'))]
                ),
                LS::Make(ModuleLang::class)->Get('error')
            );
        ...
        LS::Make(ModuleMessage::class)->AddNotice(
            LS::Make(ModuleLang::class)->Get('userfeed_subscribes_updated'),
            LS::Make(ModuleLang::class)->Get('attention')
        );
    }
