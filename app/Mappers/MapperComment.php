 ...
 * @since   1.0
 */
class MapperComment extends Mapper
{
    ...
    public function GetCommentUnique($sTargetId, $sTargetType, $sUserId, $sCommentPid, $sHash)
    {
        $sql = "SELECT comment_id FROM ".Config::Get('db.table.comment')." 
			WHERE 
				...
				";
        if ($aRow =
            $this->oDb->selectRow($sql, $sTargetId, $sTargetType, $sUserId, $sCommentPid, $sCommentPid, $sHash)
        ) {
    ...
    public function GetCommentsAll(
        ...
        if ($sTargetType == "topic") {
            ...
            $targetIdTest =
                "t.topic_id = c.target_id AND b.blog_id = t.blog_id AND t.topic_deleted = 0 AND b.blog_deleted = 0";
        } elseif ($sTargetType == "talk") {
    ...
    public function GetCommentsByArrayId($aArrayId)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $aArrayId, $aArrayId)) {
            foreach ($aRows as $aRow) {
                $aComments[] = new EntityComment($aRow);
            }
        }
     ...
     * @return int
     */
    public function GetCountCommentsByUserId($sId, $sTargetType, $aExcludeTarget = [], $aExcludeParentTarget = [])
    {
        $sql = "SELECT 
     ...
     * @return bool|int
     */
    public function AddComment(EntityComment $oComment)
    {
        ...
        if ($iId = $this->oDb->query(
            ...
            $oComment->getUserRank(),
            $oComment->getUserAvatar()
        )
        ) {
            $oComment->setId($iId);
     ...
     * @return bool|int
     */
    public function AddCommentTree(EntityComment $oComment)
    {
        $this->oDb->transaction();

        if ($oComment->getPid() and $oCommentParent = $this->GetCommentsByArrayId([$oComment->getPid()])) {
     ...
     *
     * @param \App\Entities\EntityCommentOnline $oCommentOnline Объект онлайн комментария
     *
     * @return bool|int
     */
    ...
    public function GetCommentsByFilter($aFilter, $aOrder, &$iCount, $iCurrPage, $iPerPage)
    {
        ...
        if ($aRows = $this->oDb->selectPage(
            ...
            (isset($aFilter['target_type']) and count($aFilter['target_type'])) ? $aFilter['target_type']
                : DBSIMPLE_SKIP,
    ...
    public function UpdateEditCommentData(EntityComment $oComment)
    {
        $sql = "UPDATE ".Config::Get('db.table.comment')."
        SET
        ...
        if ($oComment->getEditCount() == null
            && $this->oDb->query(
                $sql,
                0,
                $oComment->getEditDate(),
                $oComment->getId()
            ) !== false
        ) {
            return true;
        ...
        if ($this->oDb->query($sql, $oComment->getEditCount(), $oComment->getEditDate(), $oComment->getId())
            !== false
        ) {
            return true;
        }
