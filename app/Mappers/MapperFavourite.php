 ...
 * @since   1.0
 */
class MapperFavourite extends Mapper
{
    /**
    ...
    public function GetFavouritesByArray($aArrayId, $sTargetType, $sUserId)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $sUserId, $aArrayId, $sTargetType)) {
            foreach ($aRows as $aRow) {
                $aFavourites[] = new EntityFavourite($aRow);
            }
        }
     ...
     * Добавляет тег
     *
     * @param \App\Entities\EntityFavouriteTag $oTag Объект тега
     *
     * @return bool
     */
     ...
     * @return array
     */
    public function GetFavouritesByUserId($sUserId, $sTargetType, &$iCount, $iCurrPage, $iPerPage, $aExcludeTarget = [])
    {
        $sql = "			
			SELECT target_id										
			FROM ".Config::Get('db.table.favourite')."								
    ...
    public function GetGroupTags($iUserId, $sTargetType, $bIsUser, $iLimit)
    {
        ...
        $aReturnSort = [];
        if ($aRows =
            $this->oDb->select($sql, $iUserId, $sTargetType, is_null($bIsUser) ? DBSIMPLE_SKIP : $bIsUser, $iLimit)
        ) {
            foreach ($aRows as $aRow) {
            ...
            foreach ($aReturn as $aRow) {
                $aReturnSort[] = new EntityFavouriteTag($aRow);
            }
        }
    ...
    {
        $aOrderAllow = ['target_id', 'user_id', 'is_user'];
        $sOrder = '';
        ...
        if ($aRows = $this->oDb->selectPage(
            ...
            foreach ($aRows as $aRow) {
                $aResult[] = new EntityFavouriteTag($aRow);
            }
        }
