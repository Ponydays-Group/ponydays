 ...
 * @since   1.0
 */
class MapperGeo extends Mapper
{
    /**
     ...
     *
     * @return \App\Entities\EntityGeoTarget|bool
     */
    ...
    public function GetTargets($aFilter, &$iCount, $iCurrPage, $iPerPage)
    {
        if (isset($aFilter['target_id']) and !is_array($aFilter['target_id'])) {
            $aFilter['target_id'] = [$aFilter['target_id']];
        }
        ...
        if ($aRows = $this->oDb->selectPage(
            ...
            foreach ($aRows as $aRow) {
                $aResult[] = new EntityGeoTarget($aRow);
            }
        }
    ...
    public function GetGroupCountriesByTargetType($sTargetType, $iLimit)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $sTargetType, $iLimit)) {
            foreach ($aRows as $aRow) {
                $aResult[] = new EntityGeoCountry($aRow);
            }
        }
    ...
    public function GetGroupCitiesByTargetType($sTargetType, $iLimit)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $sTargetType, $iLimit)) {
            foreach ($aRows as $aRow) {
                $aResult[] = new EntityGeoCity($aRow);
            }
        }
    ...
    {
        $aOrderAllow = ['id', 'name_ru', 'name_en', 'sort'];
        $sOrder = '';
    ...
    {
        $aOrderAllow = ['id', 'name_ru', 'name_en', 'sort', 'country_id'];
        $sOrder = '';
        foreach ($aOrder as $key => $value) {
            if (!in_array($key, $aOrderAllow)) {
                unset($aOrder[$key]);
        ...
        if (isset($aFilter['country_id']) and !is_array($aFilter['country_id'])) {
            $aFilter['country_id'] = [$aFilter['country_id']];
        }
        ...
        if ($aRows = $this->oDb->selectPage(
            ...
            foreach ($aRows as $aRow) {
                $aResult[] = new EntityGeoRegion($aRow);
            }
        }
    ...
    {
        $aOrderAllow = ['id', 'name_ru', 'name_en', 'sort', 'country_id', 'region_id'];
        $sOrder = '';
        foreach ($aOrder as $key => $value) {
            if (!in_array($key, $aOrderAllow)) {
                unset($aOrder[$key]);
        ...
        if (isset($aFilter['region_id']) and !is_array($aFilter['region_id'])) {
            $aFilter['region_id'] = [$aFilter['region_id']];
        }
