...
class MapperStaticPage extends Mapper
{
    ...
    public function AddPage(EntityStaticPage $oPage)
    {
        ...
        if ($iId = $this->oDb->query(
            ...
            $oPage->getText(),
            $oPage->getDateAdd(),
            $oPage->getSeoKeywords(),
            $oPage->getSeoDescription(),
            $oPage->getActive(),
            $oPage->getMain(),
            $oPage->getSort(),
            $oPage->getAutoBr()
    ...
    public function GetPageByUrlFull($sUrlFull, $iActive)
    {
        ...
        if ($aRow = $this->oDb->selectRow($sql, $sUrlFull, $iActive)) {
            return new EntityStaticPage($aRow);
        }
    ...
    public function GetPagesByPid($sPid)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $sPid)) {
            foreach ($aRows as $aRow) {
                $aResult[] = new EntityStaticPage($aRow);
            }
        }
    ...
    public function getListOfActivePages(&$iCount, $iCurrPage, $iPerPage)
    {
        ...
        if ($aRows = $this->oDb->selectPage($iCount, $sql, ($iCurrPage - 1) * $iPerPage, $iPerPage)) {
            foreach ($aRows as $aPage) {
                $aPages[] = new EntityStaticPage($aPage);
            }
        }
