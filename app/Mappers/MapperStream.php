 ...
 * @since   1.0
 */
class MapperStream extends Mapper
{
    /**
     * Добавление события в БД
     *
     * @param \App\Entities\EntityStreamEvent $oObject
     *
     * @return int|bool
     */
    ...
    public function GetEventByTarget($sEventType, $iTargetId, $iUserId = null)
    {
        ...
        if ($aRow =
            $this->oDb->selectRow($sql, $iTargetId, $sEventType, is_null($iUserId) ? DBSIMPLE_SKIP : $iUserId)
        ) {
            return new EntityStreamEvent($aRow);
        }
    ...
    public function UpdateEvent($oObject)
    {
        ...
        return $this->oDb->query($sql, $oObject->_getData(['publish']), $oObject->getId());
    }
    ...
    public function Read($aEventTypes, $aUsersList, $iCount, $iFromId)
    {
        ...
        if ($aRows = $this->oDb->select(
            ...
            foreach ($aRows as $aRow) {
                $aReturn[] = new EntityStreamEvent($aRow);
            }
        }
