 ...
 * @since   1.0
 */
class MapperSubscribe extends Mapper
{
    /**
    ...
    public function GetSubscribeByTypeAndMail($sType, $sMail)
    {
        ...
        if ($aRow = $this->oDb->selectRow($sql, $sType, $sMail)) {
            return new EntitySubscribe($aRow);
        }
     ...
     * Обновление подписки
     *
     * @param EntitySubscribe $oSubscribe Объект подписки
     *
     * @return int
     */
    ...
    public function GetSubscribes($aFilter, $aOrder, &$iCount, $iCurrPage, $iPerPage)
    {
        ...
        if ($aRows = $this->oDb->selectPage(
            ...
            foreach ($aRows as $aRow) {
                $aResult[] = new EntitySubscribe($aRow);
            }
        }
