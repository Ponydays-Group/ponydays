 ...
 * @since   1.0
 */
class MapperTopic extends Mapper
{
    /**
     * Добавляет топик
     *
     ...
     * @return int|bool
     */
    public function AddTopicContent(EntityTopic $oTopic)
    {
    ...
    public function GetTopicsByArrayId($aArrayId)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $aArrayId, $aArrayId)) {
            foreach ($aRows as $aTopic) {
                $aTopics[] = new EntityTopic($aTopic);
            }
        }
    ...
    public function GetTopicTags($iLimit, $aExcludeTopic = [])
    {
        ...
        if ($aRows = $this->oDb->select(
            ...
            foreach ($aReturn as $aRow) {
                $aReturnSort[] = new EntityTopicTag($aRow);
            }
        }
     ...
     * @return bool
     */
    public function UpdateTopicContent(EntityTopic $oTopic)
    {
    ...
    protected function buildFilter($aFilter)
    {
        ...
        if (isset($aFilter['blog_id'])) {
            if (!is_array($aFilter['blog_id'])) {
                $aFilter['blog_id'] = [$aFilter['blog_id']];
            }
        ...
        if (isset($aFilter['topic_type'])) {
            if (!is_array($aFilter['topic_type'])) {
                $aFilter['topic_type'] = [$aFilter['topic_type']];
            }
            $sWhere .= " AND t.topic_type IN ('".join(
                    "','",
                    array_map('mysqli::real_escape_string', $aFilter['topic_type'])
                )."')";
        }
    ...
    public function GetTopicTagsByLike($sTag, $iLimit)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $sTag.'%', $iLimit)) {
            foreach ($aRows as $aRow) {
                $aReturn[] = new EntityTopicTag($aRow);
            }
        }
     ...
     * @return int
     */
    public function UpdateTopicRead(EntityTopicRead $oTopicRead)
    {
     ...
     * @return bool
     */
    public function AddTopicRead(EntityTopicRead $oTopicRead)
    {
    ...
    public function GetTopicsReadByArray($aArrayId, $sUserId)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $aArrayId, $sUserId)) {
            foreach ($aRows as $aRow) {
                $aReads[] = new EntityTopicRead($aRow);
            }
        }
    ...
    public function AddTopicQuestionVote(EntityTopicQuestionVote $oTopicQuestionVote)
    {
        ...
        if ($this->oDb->query(
                $sql,
                $oTopicQuestionVote->getTopicId(),
                $oTopicQuestionVote->getVoterId(),
                $oTopicQuestionVote->getAnswer()
            ) === 0
        ) {
            return true;
        }
    ...
    public function GetTopicsQuestionVoteByArray($aArrayId, $sUserId)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $aArrayId, $sUserId)) {
            foreach ($aRows as $aRow) {
                $aVotes[] = new EntityTopicQuestionVote($aRow);
            }
        }
    ...
    public function GetTopicPhotosByArrayId($aArrayId)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $aArrayId, $aArrayId)) {
            foreach ($aRows as $aPhoto) {
                $aReturn[] = new EntityTopicPhoto($aPhoto);
            }
        }
    ...
    public function getTopicPhotoById($iPhotoId)
    {
        ...
        if ($aPhoto) {
            return new EntityTopicPhoto($aPhoto);
        } else {
            return null;
        }
    }
     ...
     *
     * @param EntityTopicPhoto $oPhoto Объект фото к топику-фотосету
     *
     * @return bool
     */
