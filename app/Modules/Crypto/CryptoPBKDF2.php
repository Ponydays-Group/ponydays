...
class CryptoPBKDF2 extends CryptoAlgorithm
{
    public function hash(string $password, array $params): array
    {
        ...
        if (!$hash_algo) {
            $hash_algo = Config::Get('crypto.password.pbkdf2.hash_algo');
        }
        if (!$iters) {
            $iters = Config::Get('crypto.password.pbkdf2.iterations');
        }
        if (!$salt_len) {
            $salt_len = Config::Get('crypto.password.pbkdf2.salt_len');
        }
        if (!$key_len) {
            $key_len = Config::Get('crypto.password.pbkdf2.key_len');
        }
        if (!$salt) {
    ...
    public function needs_rehash(array $params): string
    {
        ...
        if ($hash_algo != Config::Get('crypto.password.pbkdf2.hash_algo')) {
            return true;
        }
        if ($iters != Config::Get('crypto.password.pbkdf2.iterations')) {
            return true;
        }
        if ($salt_len != Config::Get('crypto.password.pbkdf2.salt_len')) {
            return true;
        }
        if ($key_len != Config::Get('crypto.password.pbkdf2.key_len')) {
            return true;
        }
