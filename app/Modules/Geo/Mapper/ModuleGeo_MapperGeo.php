...
namespace App\Modules\Geo\Mapper;

use App\Modules\Geo\Entity\ModuleGeo_EntityCity;
use App\Modules\Geo\Entity\ModuleGeo_EntityCountry;
use App\Modules\Geo\Entity\ModuleGeo_EntityRegion;
use App\Modules\Geo\Entity\ModuleGeo_EntityTarget;
use Engine\Config;
use Engine\Mapper;
...
class ModuleGeo_MapperGeo extends Mapper {
	...
	public function GetTargets($aFilter,&$iCount,$iCurrPage,$iPerPage) {
		...
		if ($aRows=$this->oDb->selectPage($iCount,$sql,
			...
			foreach ($aRows as $aRow) {
				$aResult[] = new ModuleGeo_EntityTarget($aRow);
			}
		}
		return $aResult;
	}
	...
	public function GetGroupCountriesByTargetType($sTargetType,$iLimit) {
		...
		if ($aRows=$this->oDb->select($sql,$sTargetType,$iLimit)) {
			foreach ($aRows as $aRow) {
				$aResult[] = new ModuleGeo_EntityCountry($aRow);
			}
		}
		return $aResult;
	}
	...
	public function GetGroupCitiesByTargetType($sTargetType,$iLimit) {
		...
		if ($aRows=$this->oDb->select($sql,$sTargetType,$iLimit)) {
			foreach ($aRows as $aRow) {
				$aResult[] = new ModuleGeo_EntityCity($aRow);
			}
		}
		return $aResult;
	}
	...
	public function GetRegions($aFilter,$aOrder,&$iCount,$iCurrPage,$iPerPage) {
		...
		if ($aRows=$this->oDb->selectPage($iCount,$sql,
			...
			foreach ($aRows as $aRow) {
				$aResult[] = new ModuleGeo_EntityRegion($aRow);
			}
		}
		return $aResult;
	}
