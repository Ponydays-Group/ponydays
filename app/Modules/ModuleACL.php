...
class ModuleACL extends Module
{
    ...
    public function CanAddFavourite(EntityFavourite $oFavourite, EntityUser $oUser)
    {
        if ($oFavourite->getTargetType() == "talk") {
            ...
            if (in_array($oTopic->getBlogId(), LS::Make(ModuleBlog::class)->GetAccessibleBlogsByUser($oUser))
                or $oTopic->getBlog()->getType() == "open"
            ) {
                return true;
            }
            ...
            if (in_array(
                    $oComment->getTarget()->getBlogId(),
                    LS::Make(ModuleBlog::class)->GetAccessibleBlogsByUser($oUser)
                ) or $oComment->getTarget()->getBlog()->getType() == "open"
            ) {
                return true;
            }
        }
     ...
     * @return bool
     */
    public function CanCreateBlog(EntityUser $oUser)
    {
     ...
     * @return bool
     */
    public function CanAddTopic(EntityUser $oUser, EntityBlog $oBlog)
    {
        /**
     ...
     * @return bool
     */
    public function CanPostComment(EntityUser $oUser)
    {
    ...
    /**
     * Проверяет может ли пользователь создавать комментарии по времени(например ограничение максимум 1 коммент в 5
     * минут)
     *
     * @param \App\Entities\EntityUser $oUser Пользователь
     *
     * @return bool
     */
    public function CanPostCommentTime(EntityUser $oUser)
    {
        if (Config::Get('acl.create.comment.limit_time') > 0 and $oUser->getDateCommentLast()) {
            ...
            if ($oUser->getRating() < Config::Get('acl.create.comment.limit_time_rating') and ((time()
                        - $sDateCommentLast) < Config::Get('acl.create.comment.limit_time'))
            ) {
                return false;
            }
        }
     ...
     *
     * @param  EntityUser $oUser Пользователь
     *
     * @return bool
     */
    public function CanPostTopicTime(EntityUser $oUser)
    {
         ...
         */
        $aTopics = LS::Make(ModuleTopic::class)->GetLastTopicsByUserId(
            $oUser->getId(),
            Config::Get('acl.create.topic.limit_time')
        );
     ...
     * @return bool
     */
    public function CanSendTalkTime(EntityUser $oUser)
    {
        // Для администраторов ограничение по времени не действует
        if ($oUser->isAdministrator()
         ...
         * Проверяем, если топик опубликованный меньше чем acl.create.topic.limit_time секунд назад
         */
        $aTalks = LS::Make(ModuleTalk::class)->GetLastTalksByUserId(
            $oUser->getId(),
            Config::Get('acl.create.talk.limit_time')
        );
     ...
     * @return bool
     */
    public function CanPostTalkCommentTime(EntityUser $oUser)
    {
        /**
         * Для администраторов ограничение по времени не действует
         */
     ...
     * @return bool
     */
    public function CanUseHtmlInComment(EntityUser $oUser)
    {
        return true;
    }
     ...
     *
     * @param \App\Entities\EntityUser    $oUser    Пользователь
     * @param \App\Entities\EntityComment $oComment Комментарий
     *
     * @return bool
     */
    public function CanVoteComment(EntityUser $oUser, EntityComment $oComment)
    {
        ...
        if (!in_array(
                $oComment->getTarget()->getBlogId(),
                LS::Make(ModuleBlog::class)->GetAccessibleBlogsByUser($oUser)
            ) and $oComment->getTarget()->getBlog()->getType() != "open"
        ) {
            return false;
        }
     ...
     * @return bool
     */
    public function CanVoteBlog(EntityUser $oUser, EntityBlog $oBlog)
    {
        /**
     ...
     *
     * @param EntityUser                $oUser  Пользователь
     * @param \App\Entities\EntityTopic $oTopic Топик
     *
     * @return bool
     */
    public function CanVoteTopic(EntityUser $oUser, EntityTopic $oTopic)
    {
     ...
     *
     * @param \App\Entities\EntityUser $oUser       Пользователь
     * @param \App\Entities\EntityUser $oUserTarget Пользователь за которого голосуем
     *
     * @return bool
     */
    public function CanVoteUser(EntityUser $oUser, EntityUser $oUserTarget)
    {
     ...
     * @return bool
     */
    public function CanSendInvite(EntityUser $oUser)
    {
    ...
    public function IsAllowEditTopic($oTopic, $oUser)
    {
         ...
         */
        if (LS::Make(ModuleUser::class)->GetUserCurrent() and LS::Make(ModuleUser::class)->GetUserCurrent()->getId()
            == $oUser->getId()
        ) {
            /**
            ...
            $oBlogUser =
                LS::Make(ModuleBlog::class)->GetBlogUserByBlogIdAndUserId($oTopic->getBlogId(), $oUser->getId());
     ...
     * @return bool
     */
    public function IsAllowPublishIndex(EntityUser $oUser)
    {
        if ($oUser->isAdministrator()) {
            return true;
        }
     ...
     *
     * @param \App\Entities\EntityUser $oUser Пользователь
     * @param \App\Entities\EntityWall $oWall Объект сообщения на стене
     *
     * @return bool
     */
    public function CanAddWallTime($oUser, $oWall)
    {
         ...
         */
        $aWall = LS::Make(ModuleWall::class)->GetWall(['user_id' => $oWall->getUserId()], ['id' => 'desc'], 1, 1, []);
        /**
         * Если сообщений нет
         */
        if ($aWall['count'] == 0) {
...
//			($oUser->isGlobalModerator() && $oTopic->getBlog()->getType() == 'open')
        ...
        if (
            $oBlog->getUserIsAdministrator() || $oBlog->getUserIsModerator() || $oBlog->getOwnerId() == $oUser->getId()
        ) {
            return true;
        }
    ...
    public function IsAllowControlTopic($oTopic, $oUser)
    {
        return ($this::IsAllowEditTopic($oTopic, $oUser) && !$oTopic->isControlLocked())
            || $this::IsAllowLockTopicControl($oTopic, $oUser);
    }
            ...
            if ($req >= 2) {
                switch ($sTargetType) {
                    ...
                    case '__non_checkable_visible__':
                        ...
                        if ($oTarget === null) {
                            $isNonCheckableVisibleObject = true;
                        }
                        break;
                    default:
                        return false;
                ...
                if (isset($oTopic) || isset($oBlog)) {
                    if (!isset($oBlog) && isset($oTopic) && $oTopic) {
                        $oBlog = $oTopic->getBlog();
                    }
                    if ($oUser->isGlobalModerator() && $oBlog->getType() == 'open') {
                        return true;
                    }
                    ...
                    if ($req >= 5) {
                        if (in_array($oBlog->getId(), LS::Make(ModuleBlog::class)->GetAccessibleBlogsByUser($oUser))
                            || (
                                $oBlog->getType() === 'open'
                                && (
                                    !($oBlogUser = LS::Make(ModuleBlog::class)->GetBlogUserByBlogIdAndUserId(
                                        $oBlog->getId(),
                                        $oUser->getId()
                                    ))
                ...
                } elseif (isset($oTargetUser)) {
                    //return true;
                } elseif (isset($isNonCheckableVisibleObject) && $isNonCheckableVisibleObject === true) {
    ...
    function UserCanEditComment($oUser, $oComment, $iCheckMask = 0)
    {
        ...
        if ($oComment->getTargetType() != 'talk') {
            if ($oComment->getTarget()->getBlog()->getType() != 'personal') {
                $oBlog = $oComment->getTarget()->getBlog()->GetBlogUserByBlogIdAndUserId(
                    $oComment->getTarget()->getBlog()->getId(),
                    $oUser->getId()
                );
                if ($oBlog) {
        ...
        if ($iCheckMask & ModuleACL::ACL_EC_CHECK_MAX_EDIT_PERIOD != 0
            && Config::Get(
                'module.editcomment.max_edit_period'
            )
        ) {
            if (strtotime(
                    '+'.Config::Get('module.editcomment.max_edit_period').' second',
                    strtotime($oComment->getEditDate())
                ) < time()
            ) {
                return false;
            }
        ...
        if ($iCheckMask & ModuleACL::ACL_EC_CHECK_DENY_WITH_ANSWERS != 0
            && Config::Get(
                'module.editcomment.deny_with_answers'
            )
        ) {
            $oCommentAnswer = LS::Make(ModuleEditComment::class)->GetFirstAnswer($oComment->getId());
            if ($oCommentAnswer
                && (time() - strtotime($oCommentAnswer->getDate())) > Config::Get(
                    'module.editcomment.max_edit_period_from_first_answer'
                )
            ) {
                return false;
            }
    ...
    function UserCanDeleteComment($oUser, $oComment, $iCheckMask = 0)
    {
        ...
        if ($oComment->getTargetType() != 'talk') {
            if ($oComment->getTarget()->getBlog()->getType() != 'personal') {
                $oBlog = $oComment->getTarget()->getBlog()->Blog_GetBlogUserByBlogIdAndUserId(
                    $oComment->getTarget()->getBlog()->getId(),
                    $oUser->getId()
                );
                if ($oBlog) {
