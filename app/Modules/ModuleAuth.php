...
class ModuleAuth extends Module
{
    ...
    public function GenerateKey(
        ...
        $header_data = base64url_encode(
            data_format_pack(
                $data_format,
                $header
            )
        );
        $payload_data = base64url_encode(
            data_format_pack(
                $data_format,
                $payload
            )
    ...
    public function VerifyKey(string $key, string $data_format = 'json'): array
    {
        ...
        $header = data_format_unpack_type(
            $data_format,
            base64url_decode($header_data),
            "array"
        );
        if (!$header) {
            throw new AuthException(
        ...
        $payload = data_format_unpack_type(
            $data_format,
            base64url_decode($payload_data),
            "array"
        );
        if (!$payload) {
            throw new AuthException(
