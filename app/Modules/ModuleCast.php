...
class ModuleCast extends Module
{
    ...
    {
        $this->oMapper = Engine::MakeMapper(MapperCast::class);
    }
        ...
        if (!$this->oMapper->castExist($sTarget, $oTarget->getId(), $oUser->getId())) {
            ...
            if ($sTarget == "topic") {
                ...
                $notificationTitle =
                    ...
                    ."</a> упомянул вас в посте <a href='".$notificationLink."'>".$oTarget->getTitle()."</a> ";
                $notificationText = "";
                ...
                $notificationTitle =
                    "<a href='".$this->oUserCurrent->getUserWebPath()."'>".$this->oUserCurrent->getLogin()
                    ."</a> упомянул вас в <a href='".$notificationLink."'>комментарии</a> <a href='".$topicLink."'>"
                    .$oParentTarget->getTitle()."</a>";
                $notificationText = "";
                $notification = new EntityNotification(
                    [
                        'user_id'           => $oUser->getId(),
