...
class ModuleComment extends Module
{
     ...
     */
    public function GetCommentsAll($sTargetType, $iPage, $iPerPage, $aExcludeTarget = [], $aExcludeParentTarget = [])
    {
        ...
        if (false === ($data = $cache->Get("comment_all_{$sTargetType}_{$iPage}_{$iPerPage}_{$s}"))) {
            ...
            $cache->Set(
                $data,
                "comment_all_{$sTargetType}_{$iPage}_{$iPerPage}_{$s}",
                ["comment_new_{$sTargetType}", "comment_update_status_{$sTargetType}"],
                60 * 60 * 24 * 1
            );
        }
        $data['collection'] =
            $this->GetCommentsAdditionalData($data['collection'], ['target', 'favourite', 'user' => []]);
    ...
    public function GetCommentsAdditionalData($aCommentId, $aAllowData = null)
    {
        if (is_null($aAllowData)) {
            $aAllowData = ['vote', 'target', 'favourite', 'user' => [], 'delete_reason', 'delete_user_id'];
        }
        ...
        $aTargets['topic'] = LS::Make(ModuleTopic::class)->GetTopicsAdditionalData(
            $aTargetId['topic'],
            ['blog' => ['owner' => []], 'user' => [], 'comment_new']
        );
        $aVote = [];
        ...
        if (isset($aAllowData['favourite']) and $this->oUserCurrent) {
            $aFavouriteComments = LS::Make(ModuleFavourite::class)->GetFavouritesByArray(
                $aCommentId,
                'comment',
    ...
    public function GetCommentsByArrayId($aCommentId)
    {
        ...
        if ($data = $this->oMapper->GetCommentsByArrayId($aCommentIdNeedQuery)) {
            foreach ($data as $oComment) {
                ...
                $cache->Set($oComment, "comment_{$oComment->getId()}", [], 60 * 60 * 24 * 4);
        ...
        foreach ($aCommentIdNeedStore as $sId) {
            $cache->Set(null, "comment_{$sId}", [], 60 * 60 * 24 * 4);
        }
        /**
    ...
    {
        return $this->GetCommentsByArrayId(
            $this->oMapper->GetCommentsOlderThenEdited($sTargetType, $iTargetId, $iCommentId)
        );
    }
    ...
    public function GetCommentsByArrayIdSolid($aCommentId)
    {
        ...
        if (false === ($data = $cache->Get("comment_id_{$s}"))) {
            ...
            $cache->Set($aComments, "comment_id_{$s}", ["comment_update"], 60 * 60 * 24 * 1);
    ...
    public function GetCommentsByUserId($sId, $sTargetType, $iPage, $iPerPage)
    {
        ...
        if (false === ($data = $cache->Get("comment_user_{$sId}_{$sTargetType}_{$iPage}_{$iPerPage}_{$s}"))) {
            ...
            $cache->Set(
                $data,
                "comment_user_{$sId}_{$sTargetType}_{$iPage}_{$iPerPage}_{$s}",
    ...
    public function GetCountCommentsByUserId($sId, $sTargetType)
    {
        ...
        if (false === ($data = $cache->Get("comment_count_user_{$sId}_{$sTargetType}_{$s}"))) {
            ...
            $cache->Set(
                $data,
                "comment_count_user_{$sId}_{$sTargetType}",
                ["comment_new_user_{$sId}_{$sTargetType}", "comment_update_status_{$sTargetType}"],
                60 * 60 * 24 * 2
            );
        }
     ...
     *
     * @param  string $sDate       Дата за которую выводить рейтинг, т.к. кеширование происходит по дате, то дату лучше
     *                             передавать с точностью до часа (минуты и секунды как 00:00)
     * @param  string $sTargetType Тип владельца комментария
     * @param  int    $iLimit      Количество элементов
     *
     * @return array
    ...
    public function GetCommentsRatingByDate($sDate, $sTargetType, $iLimit = 20)
    {
        ...
        if (false === ($data = $cache->Get("comment_rating_{$sDate}_{$sTargetType}_{$iLimit}_{$s}"))) {
            $data = $this->oMapper->GetCommentsRatingByDate($sDate, $sTargetType, $iLimit, [], $aCloseBlogs);
            $cache->Set(
                $data,
                "comment_rating_{$sDate}_{$sTargetType}_{$iLimit}_{$s}",
                [
    ...
    public function GetCommentsByTargetId($sId, $sTargetType, $iPage = 1, $iPerPage = 0)
    {
        ...
        if (false === ($aCommentsRec = $cache->Get("comment_target_{$sId}_{$sTargetType}"))) {
            ...
            $cache->Set(
                $aCommentsRec,
                "comment_target_{$sId}_{$sTargetType}",
                ["comment_new_{$sTargetType}_{$sId}"],
                60 * 60 * 24 * 2
            );
        }
        if (!isset($aCommentsRec['comments'])) {
            return ['comments' => [], 'iMaxIdComment' => 0];
        }
    ...
    {
        if (!Config::Get('module.comment.nested_page_reverse') and $iPerPage and
            $iCountPage = ceil($this->GetCountCommentsRootByTargetId($sId, $sTargetType) / $iPerPage)
        ) {
        ...
        if (false === ($aReturn = $cache->Get("comment_tree_target_{$sId}_{$sTargetType}_{$iPage}_{$iPerPage}"))) {
            ...
            if ($iPerPage) {
                $aComments =
                    $this->oMapper->GetCommentsTreePageByTargetId($sId, $sTargetType, $iCount, $iPage, $iPerPage);
            } else {
            ...
            $aReturn = ['comments' => $aComments, 'iMaxIdComment' => $iMaxIdComment, 'count' => $iCount];
            $cache->Set(
                $aReturn,
                "comment_tree_target_{$sId}_{$sTargetType}_{$iPage}_{$iPerPage}",
                ["comment_new_{$sTargetType}_{$sId}"],
                60 * 60 * 24 * 2
            );
        }
    ...
    public function GetPageCommentByTargetId($sId, $sTargetType, $oComment)
    {
        ...
        if ($oComment->getPid()) {
            if (!($oCommentRoot =
                $this->oMapper->GetCommentRootByTargetIdAndChildren($sId, $sTargetType, $oComment->getLeft()))
            ) {
                return false;
            }
        } else {
        ...
        $iCount = ceil(
            $this->oMapper->GetCountCommentsAfterByTargetId($sId, $sTargetType, $oCommentRoot->getLeft()) / Config::Get(
                'module.comment.nested_per_page'
            )
        );

        if (!Config::Get('module.comment.nested_page_reverse') and $iCountPage = ceil(
                $this->GetCountCommentsRootByTargetId($sId, $sTargetType) / Config::Get(
                    'module.comment.nested_per_page'
                )
            )
        ) {
    ...
    public function AddComment(EntityComment $oComment, $bMark = false)
    {
        ...
        if (Config::Get('module.comment.use_nested')) {
            ...
            $cache->Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, ["comment_update"]);
        } else {
            ...
            if (strstr($oComment->getText(), "@moderator")) {
                if ($oTarget->getBlog()->getType() == "open") {
                    LS::Make(ModuleTalk::class)->SendTalk(
                        "Вызов модератора в пост ".$oTarget->getTitle(),
                        "Я <a target='_blank' href='".$oTarget->getUrl()."#comment".$sId
                        ."'>прошу</a> модераторов посмотреть пост <a href='".$oTarget->getUrl()."'>".$oTarget->getTitle(
                        )."</a> и комментарии к нему на соответствие правилам.",
                        $oComment->getUserId(),
                        Config::Get("moderator")
                    );
                } else {
                    $aModersCollection = LS::Make(ModuleBlog::class)->GetBlogUsersByBlogId(
    ...
    public function UpdateCommentStatus(EntityComment $oComment)
    {
        if ($this->oMapper->UpdateComment($oComment)) {
             ...
             * Обновляем избранное
             */
            LS::Make(ModuleFavourite::class)->SetFavouriteTargetPublish(
                $oComment->getId(),
                'comment',
                !$oComment->getDelete()
            );
             ...
             */
            LS::Order(
                function (ModuleCache $cache) use ($oComment) {
                    if (Config::Get('sys.cache.solid')) {
                        $cache->Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, ["comment_update"]);
                    }
                    $cache->Clean(
    ...
    public function SetCommentsPublish($sTargetId, $sTargetType, $iPublish)
    {
        ...
        /** @var ModuleCache $cache */
        $cache = LS::Make(ModuleCache::class);
        $cache->Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, ["comment_update_status_{$sTargetType}"]);
        /**
    ...
    public function DeleteCommentOnlineByTargetId($sTargetId, $sTargetType)
    {
        /** @var ModuleCache $cache */
        $cache = LS::Make(ModuleCache::class);
        $cache->Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, ["comment_online_update_{$sTargetType}"]);
    ...
    public function GetCommentsNewByTargetId($sId, $sTargetType, $sIdCommentLast)
    {
        ...
        if (false === ($aComments = $cache->Get("comment_target_{$sId}_{$sTargetType}_{$sIdCommentLast}"))) {
            ...
            $cache->Set(
                $aComments,
                "comment_target_{$sId}_{$sTargetType}_{$sIdCommentLast}",
                ["comment_new_{$sTargetType}_{$sId}"],
                60 * 60 * 24 * 1
            );
        }
        ...
        return ['comments' => $aCmt, 'iMaxIdComment' => $iMaxIdComment];
    }
    ...
    protected function BuildCommentsRecursive($aComments, $bBegin = true)
    {
        ...
        return ['comments' => $aResultCommnets, 'iMaxIdComment' => $iMaxIdComment];
    }
    ...
    public function DeleteCommentByTargetId($aTargetId, $sTargetType)
    {
        ...
        if (Config::Get('sys.cache.solid')) {
            ...
            $cache->Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, ["comment_target_{$sTargetId}_{$sTargetType}"]);
            /**
             * Удаляем кеш для каждого комментария
             */
            foreach ($aCommentsId as $iCommentId) {
                $cache->Delete("comment_{$iCommentId}");
            }
        }
    ...
    public function UpdateTargetParentByTargetId($sParentId, $sTargetType, $aTargetId)
    {
        ...
        /** @var ModuleCache $cache */
        $cache = LS::Make(ModuleCache::class);
        $cache->Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, ["comment_new_{$sTargetType}"]);
    ...
    {
        if (is_null($aAllowData)) {
            $aAllowData = ['target', 'user' => []];
        }
        $aCollection = $this->oMapper->GetCommentsByFilter($aFilter, $aOrder, $iCount, $iCurrPage, $iPerPage);

        return ['collection' => $this->GetCommentsAdditionalData($aCollection, $aAllowData), 'count' => $iCount];
    }
    ...
    public function ConvertCommentToArray($oComment, $sReadlast = null, $bIgnoreDelete = false)
    {
        ...
        $aComment['author'] = [
            "id"     => $oComment->getUserId(),
            "login"  => $oUser->getLogin(),
            "avatar" => $oUser->getProfileAvatarPath(48)
        ];
