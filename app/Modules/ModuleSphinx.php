...
class ModuleSphinx extends Module
{
    ...
    protected function InitSphinx()
    {
        ...
        $this->oSphinx->SetServer(
            Config::Get('module.search.sphinx.host'),
            intval(Config::Get('module.search.sphinx.port'))
        );
        /**
    ...
    public function FindContent($sTerms, $sObjType, $iOffset, $iLimit, $aExtraFilters)
    {
        ...
        $cacheKey = Config::Get('module.search.entity_prefix')
            ."searchResult_{$sObjType}_{$sTerms}_{$iOffset}_{$iLimit}_{$sExtraFilters}";
        if (false === ($data = $cache->Get($cacheKey))) {
            /**
             * Параметры поиска
             */
            ...
            if (!is_null($aExtraFilters)) {
                foreach ($aExtraFilters AS $sAttribName => $sAttribValue) {
                    $this->oSphinx->SetFilter(
                        $sAttribName,
                        (is_array($sAttribValue)) ? $sAttribValue : [$sAttribValue]
                    );
                }
            }
            /**
             ...
             */
            if (!is_array(
                $data = $this->oSphinx->Query($sTerms, Config::Get('module.search.entity_prefix').$sObjType.'Index')
            )
            ) {
    ...
    public function GetSnippet($sText, $sIndex, $sTerms, $before_match, $after_match)
    {
        $aReturn = $this->oSphinx->BuildExcerpts(
            [$sText],
            Config::Get('module.search.entity_prefix').$sIndex.'Index',
            $sTerms,
            [
