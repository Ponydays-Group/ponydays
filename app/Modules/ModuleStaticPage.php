...
class ModuleStaticPage extends Module
{
    ...
    public function AddPage(EntityStaticPage $oPage)
    {
        if ($sId = $this->oMapper->AddPage($oPage)) {
            ...
            LS::Make(ModuleCache::class)->Clean(
                ...
                ['page_change', "page_change_{$oPage->getId()}", "page_change_urlfull_{$oPage->getUrlFull()}"]
            );
    ...
    public function GetPageByUrlFull($sUrlFull, $iActive = 1)
    {
        ...
        if (false === ($data = $cache->Get("page_{$sUrlFull}_{$iActive}"))) {
            ...
            if ($data) {
                $cache->Set($data, "page_{$sUrlFull}_{$iActive}", ["page_change_{$data->getId()}"], 60 * 60 * 24 * 5);
            } else {
                $cache->Set(
                    $data,
                    "page_{$sUrlFull}_{$iActive}",
                    ["page_change_urlfull_{$sUrlFull}"],
                    60 * 60 * 24 * 5
                );
            }
        }
