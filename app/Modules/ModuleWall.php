...
class ModuleWall extends Module
{
    ...
    public function GetWallAdditionalData($aWallId, $aAllowData = null)
    {
        if (is_null($aAllowData)) {
            $aAllowData = ['user' => [], 'wall_user' => [], 'reply'];
        }
        ...
        $aWallUsers =
            isset($aAllowData['wall_user']) && is_array($aAllowData['wall_user']) ? LS::Make(ModuleUser::class)
                ->GetUsersAdditionalData($aWallUserId, $aAllowData['wall_user'])
                : LS::Make(ModuleUser::class)->GetUsersAdditionalData($aWallUserId);
        $aWallReply = [];
        if (isset($aAllowData['reply']) and count($aWallReplyId)) {
            $aWallReply = $this->GetWallAdditionalData($aWallReplyId, ['user' => []]);
        }
        /**
    ...
    public function UpdatePidWall($oWall, $iLimit = null)
    {
        ...
        $aResult = $this->GetWall(['pid' => $oWall->getId()], ['id' => 'desc'], 1, $iLimit, []);
        if ($aResult['count']) {
