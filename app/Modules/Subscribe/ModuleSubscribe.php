...
use App\Modules\Subscribe\Mapper\ModuleSubscribe_MapperSubscribe;
...
class ModuleSubscribe extends Module {
	...
	public function Init() {
		$this->oMapper=Engine::MakeMapper(ModuleSubscribe_MapperSubscribe::class);
	...
	public function AddSubscribeSimple($sTargetType,$sTargetId,$sMail) {
		...
		if (!($oSubscribe=$sub->GetSubscribeByTargetAndMail($sTargetType,$sTargetId,$sMail))) {
			$oSubscribe = new ModuleSubscribe_EntitySubscribe();
	...
	public function Send($sTargetType,$iTargetId,$sTemplate,$sTitle,$aParams=array(),$aExcludeMail=array(),$sPluginName=null) {
		...
		/** @var ModuleSubscribe $sub */
		$sub = LS::Make(ModuleSubscribe::class);
		$aSubscribes=$sub->GetSubscribes(array('target_type'=>$sTargetType,'target_id'=>$iTargetId,'status'=>1,'exclude_mail'=>$aExcludeMail),array(),$iPage,20);
		while ($aSubscribes['collection']) {
			...
			$aSubscribes=$sub->GetSubscribes(array('target_type'=>$sTargetType,'target_id'=>$iTargetId,'status'=>1),array(),$iPage,20);
		}
	}
	/**
	...
	public function CheckTargetTopicNewComment($iTargetId,$iStatus) {
		if ($oTopic=LS::Make(ModuleTopic::class)->GetTopicById($iTargetId)) {
			...
			if ($iStatus==1 and $oTopic->getBlog()->getType()=='close') {
				if (!$this->oUserCurrent or !($oTopic->getBlog()->getOwnerId()==$this->oUserCurrent->getId() or LS::Make(ModuleBlog::class)->GetBlogUserByBlogIdAndUserId($oTopic->getBlogId(),$this->oUserCurrent->getId()))) {
					return false;
				}
			}
			return true;
