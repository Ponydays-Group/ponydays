...
class ModuleTopic_EntityTopic extends Entity {
	...
	public function Init() {
		parent::Init();
		$this->lang = LS::Make(ModuleLang::class);
		$this->aValidateRules[]=array('topic_title','string','max'=>200,'min'=>2,'allowEmpty'=>false,'label'=>$this->lang->Get('topic_create_title'),'on'=>array('topic','link','photoset'));
		$this->aValidateRules[]=array('topic_title','string','max'=>200,'min'=>2,'allowEmpty'=>false,'label'=>$this->lang->Get('topic_question_create_title'),'on'=>array('question'));
		$this->aValidateRules[]=array('topic_text_source','string','max'=>Config::Get('module.topic.max_length'),'min'=>2,'allowEmpty'=>false,'label'=>$this->lang->Get('topic_create_text'),'on'=>array('topic','photoset'));
		$this->aValidateRules[]=array('topic_text_source','string','max'=>Config::Get('module.topic.link_max_length'),'min'=>10,'allowEmpty'=>false,'label'=>$this->lang->Get('topic_create_text'),'on'=>array('link'));
		$this->aValidateRules[]=array('topic_text_source','string','max'=>Config::Get('module.topic.question_max_length'),'allowEmpty'=>true,'label'=>$this->lang->Get('topic_create_text'),'on'=>array('question'));
		$this->aValidateRules[]=array('topic_tags','tags','count'=>15,'label'=>$this->lang->Get('topic_create_tags'),'allowEmpty'=>Config::Get('module.topic.allow_empty_tags'),'on'=>array('topic','link','question','photoset'));
		...
		$this->aValidateRules[]=array('link_url','url','allowEmpty'=>false,'label'=>$this->lang->Get('topic_link_create_url'),'on'=>array('link'));
	}
	/**
	...
	public function ValidateTopicType($sValue,$aParams) {
		if (LS::Make(ModuleTopic::class)->IsAllowTopicType($sValue)) {
			return true;
		}
		return $this->lang->Get('topic_create_type_error');
	}
	/**
	...
	public function ValidateTopicUnique($sValue,$aParams) {
		...
		if ($oTopicEquivalent=LS::Make(ModuleTopic::class)->GetTopicUnique($this->getUserId(),$this->getTextHash())) {
			if ($iId=$this->getId() and $oTopicEquivalent->getId()==$iId) {
				return true;
			}
			return $this->lang->Get('topic_create_text_error_unique');
		}
		return true;
	}
	/**
	...
	public function ValidateBlogId($sValue,$aParams) {
		...
		if (LS::Make(ModuleBlog::class)->GetBlogById((string)$sValue)) {
			return true;
		}
		return $this->lang->Get('topic_create_blog_error_unknown');
	}
	...
	public function getUser() {
		if (!$this->_getDataOne('user')) {
			$this->_aData['user']=LS::Make(ModuleUser::class)->GetUserById($this->getUserId());
		}
	...
	public function getIsAllowDelete() {
		if ($oUser=LS::Make(ModuleUser::class)->GetUserCurrent()) {
			return LS::Make(ModuleACL::class)->IsAllowDeleteTopic($this,$oUser);
		}
		return false;
	}
	/**
	...
	public function getIsAllowEdit() {
		if ($oUser=LS::Make(ModuleUser::class)->GetUserCurrent()) {
			return LS::Make(ModuleTopic::class)->IsAllowEditTopic($this,$oUser);
		}
		return false;
	}
	/**
	...
	public function getSubscribeNewComment() {
		...
		return LS::Make(ModuleSubscribe::class)->GetSubscribeByTargetAndMail('topic_new_comment',$this->getId(),$oUserCurrent->getMail());
	}
	/**
	...
	public function getPhotosetPhotos($iFromId = null, $iCount = null) {
		return LS::Make(ModuleTopic::class)->getPhotosByTopicId($this->getId(), $iFromId, $iCount);
	}
	/**
	...
	public function testAllowLockControl($oUser) {
		return LS::Make(ModuleACL::class)->IsAllowLockTopicControl($this, $oUser);
	}
}
