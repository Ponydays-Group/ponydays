...
class ModuleAuth extends Module
{
	 ...
	 */
	public function GenerateKey(array $payload, string $sign_key, string $sign_algo, string $data_format = 'json'): string
	{
	 ...
	 */
	public function VerifyKey(string $key, string $sign_key, string $data_format = 'json'): array
	{
		$calltime = time();
		...
		$signature = data_format_unpack($data_format, base64url_decode($signature_data));
		...
		if(!$signature_algo->verify($header . $payload, $signature, $sign_key, array('hash' => $algo[1]))) {
