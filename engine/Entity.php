...
abstract class Entity extends LsObject
{
     ...
     *
     * @param array|null $aKeys Список полей, данные по которым необходимо вернуть, если не передан, то возвращаются
     *                          все данные
     *
     * @return array
     */
    ...
    public function __call($sName, $aArgs)
    {
        ...
        if (!strpos($sName, '_') and in_array($sType, ['get', 'set'])) {
    ...
    public function _createValidators()
    {
        ...
        foreach ($this->aValidateRules as $aRule) {
            if (isset($aRule[0], $aRule[1])) {
                $aValidators[] = LS::Make(ModuleValidate::class)->CreateValidator(
                    $aRule[1],
                    $this,
                    $aRule[0],
                    array_slice($aRule, 2)
     ...
     * либо указан пустой сценарий '' Если указать сценарий, то проверка будет только по правилом, где в списке
     * сценарией есть указанный
     *
     * @param string $sValue
     */
