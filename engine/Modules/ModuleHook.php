 ...
 * {/hookb}
 * </pre>
 * Они позволяют заменить содержимое между {hookb ..} {/hookb} или добавть к нему произвольный контент. К имени такого
 * хука добавляется префикс "template_block_"
 * <pre>
 * class HookTest extends Hook {
 *    // Регистрируем хуки (вешаем обработчики)
 *    public function RegisterHook() {
...
class ModuleHook extends Module
{
     ...
     * @return bool
     */
    public function Add($sName, $sType, $sCallBack, $iPriority = 1, $aParams = [])
    {
        $sName = strtolower($sName);
        $sType = strtolower($sType);
        if (!in_array($sType, ['module', 'hook', 'function'])) {
            return false;
        }
        $this->aHooks[$sName][] =
            ['type' => $sType, 'callback' => $sCallBack, 'params' => $aParams, 'priority' => (int)$iPriority];
     ...
     * @return bool
     */
    public function AddExecHook($sName, $sCallBack, $iPriority = 1, $aParams = [])
    {
     ...
     * Делегирующий хук применяется для перекрытия метода модуля, результат хука возвращает вместо результата метода
     * модуля Позволяет в качестве обработчика использовать метод модуля
     *
     * @see Add
    ...
    {
        return $this->Add($sName, 'module', $sCallBack, $iPriority, ['delegate' => true]);
    }

    /**
     * Добавляет делегирующий обработчик хука с типом "function"
     * Делегирующий хук применяется для перекрытия метода модуля, результат хука возвращает вместо результата метода
     * модуля Позволяет в качестве обработчика использовать функцию
     *
     * @see Add
     *
     * @param string $sName     Имя хука
    ...
    {
        return $this->Add($sName, 'function', $sCallBack, $iPriority, ['delegate' => true]);
    }

    /**
     * Добавляет делегирующий обработчик хука с типом "hook"
     * Делегирующий хук применяется для перекрытия метода модуля, результат хука возвращает вместо результата метода
     * модуля Позволяет в качестве обработчика использовать метод хука(класса хука из каталога /classes/hooks/)
     *
     * @see Add
     * @see Hook::AddHook
     *
     ...
     * @return bool
     */
    public function AddDelegateHook($sName, $sCallBack, $iPriority = 1, $aParams = [])
    {
     ...
     * @return array
     */
    public function RunRef($sName, &$aVars = [])
    {
        $result = [];
        $sName = strtolower($sName);
        ...
        if (isset($this->aHooks[$sName])) {
            ...
            for ($i = 0; $i < count($this->aHooks[$sName]); $i++) {
                if (isset($this->aHooks[$sName][$i]['params']['delegate'])
                    and $this->aHooks[$sName][$i]['params']['delegate']
                ) {
    ...
    protected function RunType($aHook, &$aVars)
    {
        ...
        switch ($aHook['type']) {
            case 'module':
                $result = call_user_func_array([$this, $aHook['callback']], [&$aVars]);
                break;
            case 'function':
                $result = call_user_func_array($aHook['callback'], [&$aVars]);
                break;
            case 'hook':
                ...
                if ($sHookClass and class_exists($sHookClass)) {
                    ...
                    $result = call_user_func_array([$oHook, $aHook['callback']], [&$aVars]);
                }
                break;
            default:
                break;
