...
require_once './lib/LiveImage/Image.php';
...
class ModuleImage extends Module
{
     ...
     * @param  string         $sDirDest       Директория куда нужно сохранить изображение относительно корня сайта
     *                                        (path.root.server)
     ...
     * @param  bool           $bForcedMinSize Растягивать изображение по ширине или нет, если исходное меньше. При
     *                                        false - изображение будет растянуто
    ...
    public function Resize(
        ...
        if ($iWidthDest) {
            ...
            if ($aParams['watermark_use']) {
                if ($oImage->get_image_params('width') > $aParams['watermark_min_width'] and $oImage->get_image_params(
                        'height'
                    ) > $aParams['watermark_min_height']
                ) {
                    switch ($aParams['watermark_type']) {
                        default:
                        case 'text':
                            $oImage->set_font(
                                $aParams['watermark_font_size'],
                                0,
    ...
    {
        $sFileDestFullPath = rtrim(Config::Get('static_server'), "/").'/'.trim($sDirDest, "/").'/'.$sFileDest;
    ...
    {
        @func_mkdir(Config::Get('static_server'), $sDirDest);
    }
    ...
    public function GetServerPath($sPath)
    {
        ...
        if ($bStatic) {
            return rtrim(Config::Get('static_server'), '/').'/'.$sPath;
        } else {
    ...
    public function GetWebPath($sPath)
    {
        ...
        if ($bServer) {
            ...
            $sServerPath = $sStatic;
            $sWebPath = rtrim(Config::Get('static_web'), '/');
        }
...
//		return Config::Get('path.uploads.images').'/'.preg_replace('~(.{2})~U', "\\1/", str_pad($sId, 6, "0", STR_PAD_LEFT)).date('Y/m/d');
    ...
    public function BuildHTML($sPath, $aParams)
    {
        ...
        if (isset($aParams['align']) and in_array($aParams['align'], ['left', 'right', 'center'])) {
