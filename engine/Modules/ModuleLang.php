...
class ModuleLang extends Module
{
    ...
    protected function InitLang()
    {
        ...
        if (Config::Get('sys.cache.type') == 'memory') {
            if (false === ($this->aLangMsg = $cache->Get("lang_{$this->sCurrentLang}_".Config::Get('view.skin')))) {
                ...
                $cache->Set($this->aLangMsg, "lang_{$this->sCurrentLang}_".Config::Get('view.skin'), [], 60 * 60);
            }
        } else {
            $this->LoadLangFiles($this->sDefaultLang);
            if ($this->sCurrentLang != $this->sDefaultLang) {
            ...
            while (false !== ($sDirModule = readdir($hDirConfig))) {
                if ($sDirModule != '.' and $sDirModule != '..' and is_dir($sDirConfig.$sDirModule)) {
                    ...
                    if (file_exists($sFileConfig)) {
                        $this->AddMessages(include($sFileConfig), ['category' => 'module', 'name' => $sDirModule]);
                    }
                }
            }
            closedir($hDirConfig);
     ...
     * @return string
     */
    public function Get($sName, $aReplace = [], $bDelete = true)
    {
        if (strpos($sName, '.')) {
            $sLang = $this->aLangMsg;
            $aKeys = explode('.', $sName);
     ...
     * @param array|null $aParams   Параметры, позволяют хранить текстовки в структурированном виде, например, тестовки
     *                              плагина "test" получать как Get('plugin.name.test')
     */
