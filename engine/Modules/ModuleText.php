 ...
 * <pre>
 * $sText=LS::Make(ModuleText::class)->Parser($sTestSource);
 * </pre>
...
class ModuleText extends Module
{
        ...
        if (is_array($aConfig)) {
            foreach ($aConfig as $sMethod => $aExec) {
                foreach ($aExec as $aParams) {
                    if (in_array(
                        strtolower($sMethod),
                        array_map("strtolower", ['cfgSetTagCallbackFull', 'cfgSetTagCallback'])
                    )
                    ) {
                    ...
                    call_user_func_array([$this->oJevix, $sMethod], $aParams);
                }
            }
            /**
    ...
    public function VideoParser($sText)
    {
        ...
        $sText = preg_replace(
            '/<video>(?:http(?:s|):|)\/\/coub\.com\/view\/(.*)<\/video>/Ui',
            '<iframe src="https://coub.com/embed/$1" allowfullscreen="true" frameborder="0" width="480" height="270"></iframe>',
            $sText
        );
        /**
         * vimeo.com
         ...
         */
        $sText = preg_replace(
            '/<video>http:\/\/vk\.com\/(.*)<\/video>/Ui',
            '<iframe src="https://vk.com/$1" width="607" height="360" frameborder="0"></iframe>',
            $sText
        );
    ...
    public function Parser($sText)
    {
        ...
        $sResult = preg_replace(
            '/<iframe src=\"http:\/\/vk\.com\/(.*)\"(.*)\"><\/iframe>/Ui',
            '<video>http://vk.com/$1</video>',
            $sResult
        );
    ...
    protected function FlashParamParser($sText)
    {
        ...
        if (preg_match_all("@(<object\s.*>)@Ui", $sText, $aMatch)) {
            foreach ($aMatch[1] as $key => $str) {
                $sText = str_replace(
                    $aMatch[0][$key],
                    $aMatch[0][$key].'<param name="wmode" value="opaque"></param>',
                    $sText
                );
            }
        }
    ...
    public function Cut($sText)
    {
        ...
        return [$sTextShort, $sTextNew, $sTextCut ? htmlspecialchars($sTextCut) : null];
    }
