...
class ModuleViewer extends Module
{
    ...
    public function Init($bLocal = false)
    {
        ...
        $this->oSmarty->error_reporting =
            error_reporting() & ~E_NOTICE; // подавляем NOTICE ошибки - в этом вся прелесть смарти )
        $this->oSmarty->setTemplateDir(
            array_merge((array)Config::Get('path.smarty.template'), [Config::Get('path.root.server').'/plugins/'])
        );
        ...
        $this->oSmarty->addPluginsDir([Config::Get('path.smarty.plug'), 'plugins']);
     ...
     * @param bool   $bValidate               Производить или нет валидацию формы через
     *                                        {@link Security::ValidateSendForm}
     */
     ...
     *                          Можно передать название блока, тогда для обработки данных блока будет вызван обработчик
     *                          из /classes/blocks/, либо передать путь до шаблона, тогда будет выполнено обычное
     *                          подключение шаблона
     ...
     * @return bool
     */
    public function AddBlock($sGroup, $sName, $aParams = [], $iPriority = 5)
    {
        /**
        ...
        foreach ($aBlocks as $sBlock) {
            if (is_array($sBlock)) {
                $this->AddBlock(
                    $sGroup,
                    $sBlock['block'],
                    isset($sBlock['params']) ? $sBlock['params'] : [],
    ...
    protected function DefineTypeBlock($sName, $sDir = null)
    {
        if ($this->TemplateExists(
            is_null($sDir) ? 'blocks/block.'.$sName.'.tpl' : rtrim($sDir, '/').'/blocks/block.'.$sName.'.tpl'
        )
        ) {
            /**
                    ...
                    foreach ((array)$aBlocks as $sName => $aParams) {
                        ...
                        if (!is_array($aParams)) {
                            ...
                            $this->AddBlock(
                                $sGroup,
                                $sName,
                                isset($aParams['params']) ? $aParams['params'] : [],
    ...
    protected function SortBlocks()
    {
        ...
        foreach ($this->aBlocks as $sGroup => $aBlocks) {
            uasort($aBlocks, [&$this, '_SortBlocks']);
    ...
    public function MakePaging($iCount, $iCurrentPage, $iCountPerPage, $iCountPageLine, $sBaseUrl, $aGetParamsList = [])
    {
        ...
        if (is_string($aGetParamsList) or count($aGetParamsList)) {
            $sGetParams =
                '?'.(is_array($aGetParamsList) ? http_build_query($aGetParamsList, '', '&') : $aGetParamsList);
        }
        $aPaging = [
    ...
    public function SmartyDefaultTemplateHandler($sType, $sName, &$sContent, &$iTimestamp, $oSmarty)
    {
        ...
        if (Config::Get('view.skin') != 'default') {
            ...
            if (preg_match(
                '@^'.preg_quote(Config::Get('path.root.server')).'/plugins/([\w\-_]+)/templates/skin/'.preg_quote(
                ...
                $sFile = str_replace(
                    $aMatch[0],
                    Config::Get('path.root.server').'/plugins/'.$aMatch[1].'/templates/skin/default/',
                    $sName
                );
            ...
            if (preg_match(
                '@^([\w\-_]+)/templates/skin/'.preg_quote(Config::Get('view.skin')).'/@i',
                $sName,
                $aMatch
            )
            ) {
                $sFile = Config::Get('path.root.server').'/plugins/'.str_replace(
                        $aMatch[0],
                        $aMatch[1].'/templates/skin/default/',
                        $sName
                    );
                if ($this->TemplateExists($sFile)) {
                    return $sFile;
