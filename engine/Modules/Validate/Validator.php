...
abstract class Validator
{
    ...
    {
        return $mValue === null || $mValue === [] || $mValue === ''
            || $bTrim && is_scalar($mValue)
            && trim($mValue) === '';
    }
     ...
     * @param null|string $sMsgFieldCustom Поле/параметр в котором может храниться кастомное сообщение. В поле
     *                                     $sMsgFieldCustom."Id" можно хранить ключ текстовки из языкового файла
     * @param array       $aReplace        Список параметров для замены в сообщении (плейсхолдеры)
     *
     * @return string
     */
    protected function getMessage($sMsgDefault, $sMsgFieldCustom = null, $aReplace = [])
    {
        if (!is_null($sMsgFieldCustom)) {
            if (!is_null($this->$sMsgFieldCustom)) {
                ...
                if (property_exists($this, $sMsgFieldCustomId) and !is_null($this->$sMsgFieldCustomId)) {
                    $sMsgDefault = LS::Make(ModuleLang::class)->Get($this->$sMsgFieldCustomId, [], false);
                }
            }
        }
        if ($aReplace) {
    ...
    public function validateEntityField($oEntity, $sField)
    {
         ...
         */
        $sValue = call_user_func_array([$oEntity, 'get'.func_camelize($sField)], []);
    ...
    protected function getValueOfCurrentEntity($sField)
    {
        if ($this->oEntityCurrent) {
            return call_user_func_array([$this->oEntityCurrent, 'get'.func_camelize($sField)], []);
        }
    ...
    {
        if ($this->oEntityCurrent) {
            call_user_func_array([$this->oEntityCurrent, 'set'.func_camelize($sField)], [$sValue]);
        }
    }
}
