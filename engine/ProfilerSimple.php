...
class ProfilerSimple
{
    ...
    public function Start($sName, $sComment = '')
    {
        ...
        $this->iTimeId++;
        $this->aTimes[$this->sRequestId.$this->iTimeId] = [
    ...
    public function Save()
    {
        ...
        if ($fp = fopen($this->sFileName, "a")) {
            foreach ($this->aTimes as $aTime) {
                ...
                $s = date('Y-m-d H:i:s')
                    ."\t{$aTime['request_id']}\t{$aTime['time_full']}\t{$aTime['time_start']}\t{$aTime['time_stop']}\t{$aTime['time_id']}\t{$aTime['time_pid']}\t{$aTime['time_name']}\t{$aTime['time_comment']}\r\n";
                fwrite($fp, $s);
            }
            fclose($fp);
