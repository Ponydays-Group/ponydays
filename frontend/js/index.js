...
window.checkPerm = function(key) {
    const perm = localStorage.getItem(key);
...
const extend = function(self, obj) {
    for(var i in obj) {
...
async function handler(e) {
    const profile_exp = /\/profile\/[a-zA-Zа-яА-Я_0-9]+$/;
    const profile_exp_s = /\/profile\/[a-zA-Zа-яА-Я_0-9]+\/$/;
    // const blog_exp = /\/blog\/[a-zA-Z]+$/;
    // const blog_exp_s = /\/blog\/[a-zA-Z]+\/$/;
    const topic_exp = /\/blog\/[a-zA-Z]+\/\d+$/;
    // const talk_exp = /\/talk\/read\/\d+$/;
    // const talk_exp_s = /\/talk\/read\/\d+\/$/;
    const comment_exp = /#comment\d+$/;
    ...
    if(e.target.tagName == "A" || $(e.target).parents("a").length) {
        ...
        const s = e.target.getAttribute("href") || $(e.target).parents("a")[0].getAttribute("href");
        if(!s) return;
        ...
        if((info.currentWindow == $(e.target).data("floatwindow")) && info.visible) {
            const el = $(`[data-floatwindowid="${$(e.target).data("floatwindow")}"]`);
