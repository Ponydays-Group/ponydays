...
class PluginFeedbacks_HookFeedbacks extends Hook{
	...
	public function InsertUserbarItem(){
		if($this->User_GetUserCurrent()){
			$iUnreadActionsCount	= $this->PluginFeedbacks_Feedbacks_GetCurrentUserUnreadItemsCount();
			
			if($iUnreadActionsCount > 0){
				$this->Viewer_Assign('iUnreadActionsCount', $iUnreadActionsCount);
				return $this->Viewer_Fetch(Plugin::GetTemplatePath('feedbacks').'userbar_item.tpl');
			}
		}
	}
	...
	public function InsertNavbarItem(){
		if($this->User_GetUserCurrent()){
			$iUnreadActionsCount	= $this->PluginFeedbacks_Feedbacks_GetCurrentUserUnreadItemsCount();
				...
				$this->Viewer_Assign('iUnreadActionsCount', $iUnreadActionsCount);
				return $this->Viewer_Fetch(Plugin::GetTemplatePath('feedbacks').'navbar_item.tpl');
		}
	}
	...
	public function InsertNavbarStream(){
		if($this->User_GetUserCurrent()){
				return $this->Viewer_Fetch(Plugin::GetTemplatePath('feedbacks').'navbar_stream.tpl');
			}
		}
