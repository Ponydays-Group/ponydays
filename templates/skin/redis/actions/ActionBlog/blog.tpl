...
{if $oUserCurrent and $oUserCurrent->isAdministrator()}
	<div id="blog_delete_form" class="modal modal-blog-delete">
...
<div class="blog {if $bInTrash}trash{/if}">
	...
	<div class="blog-more-content" id="blog-more-content" style="display: none;">
		...
		<footer class="blog-footer">
			...
			<a href="{$oUserOwner->getUserWebPath()}" class="user"><i class="icon-user material-icons">person</i>{$oUserOwner->getLogin()}</a>
            {if $aBlogAdministrators}
            {foreach from=$aBlogAdministrators item=oBlogUser}
                {assign var="oUser" value=$oBlogUser->getUser()}
				<a href="{$oUser->getUserWebPath()}" class="user"><i class="icon-user material-icons">person</i>{$oUser->getLogin()}</a>,
            {/foreach}
            {/if}<br />
