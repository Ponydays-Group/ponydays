<section class="block block-type-foldable block-type-talk-friends">
    <header class="block-header">
        <h3><a href="#" class="link-dotted"
               onclick="jQuery('#block_talk_friends_content').toggle(); return false;">{$aLang.block_friends}</a></h3>
    </header>
            ...
            <ul class="list" id="friends">
                {foreach from=$aUsersFriend item=oFriend}
                    <li>
						<span class="checkbox">
                            <span>
                                <input id="talk_friend_{$oFriend->getId()}" type="checkbox" name="friend[{$oFriend->getId()}]"
                                       class="input-checkbox"/>

                                <label for="talk_friend_{$oFriend->getId()}"
                                    id="talk_friend_{$oFriend->getId()}_label">{$oFriend->getLogin()}</label>
                            </span>
                        </span>
                    </li>
                {/foreach}
