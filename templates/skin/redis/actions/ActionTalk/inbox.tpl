    ...
    <div id="list_wrapper">
        <form action="{router page='talk'}" method="post" id="form_talks_list">
            ...
            <button type="submit" onclick="ls.talk.makeReadTalks()"
                    class="button">{$aLang.talk_inbox_make_read}</button>
            <button type="submit" onclick="if (confirm('{$aLang.talk_inbox_delete_confirm}')){ ls.talk.removeTalks() };"
                    class="button">{$aLang.talk_inbox_delete}</button>
            <br/><br/>
                ...
                <table class="table table-talk">
                    ...
                    <tr>
                        <th class="cell-checkbox">{* <span class="checkbox"> <span><label for="select_all"></label><input type="checkbox" name="select_all" class="input-checkbox" onclick="ls.tools.checkAll('form_talks_checkbox', this, true);"></span></span> *}
                            <span class="checkbox">
											<span>
													<input id="select_all_talks" type="checkbox" name="select_all_talks"
                    ...
                    {foreach from=$aTalks item=oTalk}
                        ...
                        <tr>
							...
							<span class="checkbox">
												<span>
														<input type="checkbox" id="talk_select[{$oTalk->getId()}]"
                                                               name="talk_select[{$oTalk->getId()}]"
                                                               class="form_talks_checkbox input-checkbox"/>
                            ...
                            <td class="cell-favourite">
                                <a href="#" onclick="return ls.favourite.toggle({$oTalk->getId()},this,'talk');"
                                   class="favourite {if $oTalk->getIsFavourite()}active{/if}"></a>
                            </td>
                            <td>
                                {foreach from=$oTalk->getTalkUsers() item=oTalkUser name=users}
                                    {if $oTalkUser->getUserId()!=$oUserCurrent->getId()}
                                        ...
                                        <a href="{$oUser->getUserWebPath()}"
                                           class="user {if $oTalkUser->getUserActive()!=$TALK_USER_ACTIVE}inactive{/if}">{$oUser->getLogin()}</a>
                                    {/if}
                                {/foreach}
                            </td>
                            <td>
