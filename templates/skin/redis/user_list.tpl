			...
			{foreach from=$aUsersList item=oUserList}
				...
				<tr>
					...
					<td class="cell-date">{if $oSession}{date_format date=$oSession->getDateLast() format="d.m.y, H:i"} 
						...
						{/if}{/if}
					{/if}</td>
					{if $oUserCurrent}{if $oUserCurrent->isAdministrator() || $oUserCurrent->isGlobalModerator()}<td class="cell-email"><a href="mailto:{$oUserList->getMail()}">{$oUserList->getMail()}</a></td>{/if}{/if}
					...
					<td class="cell-rating {if $oUserList->getRating()>0}cell-rating-positive{else}cell-rating-negative{/if}"><strong>{$oUserList->getRating()}</strong></td>
				</tr>
			{/foreach}
		{else}
			<tr>
